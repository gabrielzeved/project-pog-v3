...
export class InputMouseManager implements InputSystemInterface {
	...
	isButton(button: MouseButton | string) {
		...
		return this.state[button] && this.state[button] !== MouseEventAction.MOUSE_UP;
	}
	...
	handle(event: MouseEvent): void {
		...
		if (event.type === 'mousemove') {
			...
			var x = event.clientX - rect.x; //x position within the element.
			var y = event.clientY - rect.y; //y position within the element.
